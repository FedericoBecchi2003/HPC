...
//        double pi1 = sqrt(1-x*x);    // f1
     ...
     while (i != 0) {
   	 if (threadIdx.x < i)
	 shr2[threadIdx.x] += shr2[threadIdx.x + i];
	 __syncthreads();
	 i /= 2;
     }

     if (threadIdx.x==0) res2[blockIdx.x] = shr2[threadIdx.x];
}
...
int main(int argc, char **argv ) {
    ...
    float *res2=(float*)malloc(nblocks*sizeof(float));
    float *dev_res2;
    cudaMalloc( (void**)&dev_res2, nblocks*sizeof(float) );
    ...
    cudaMemcpy( res2, dev_res2, nblocks*sizeof(float), cudaMemcpyDeviceToHost ); 

    clock_gettime( CLOCK_REALTIME ,          &t3) ;

    float total2=0;
    for (int i=0;i<nblocks;i++)    total2+=res2[i];
