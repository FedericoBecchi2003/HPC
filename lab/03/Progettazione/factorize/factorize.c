...
{
  struct timespec t_start, t_search_start, t_search_end, t_end;
  double setup_time, search_time, total_time;
  ...
  options(argc, argv);
  if (!strcmp(m, ""))
    strcpy(m, "B81915BC0A2222F4B");

  modulus_bit = strlen(m) * 4; // lunghezza hex del modulo
  BN_hex2bn(&M, m);

  block_addr_size = pow(2, block_addr_bit); // numero di blocchi da espolare
  if (block_addr_start)
    block_addr_size = block_addr_start + 1; // for checkpointing
  ...
  clock_gettime(CLOCK_REALTIME, &t_search_start);
  setup_time = (double)(t_search_start.tv_sec - t_start.tv_sec) + (double)(t_search_start.tv_nsec - t_start.tv_nsec) / BILLION;
  ...
  for (block_idx = block_addr_size - 1; block_idx > -1; block_idx--)
  {
    ...
    time = (double)(t2.tv_sec - t1.tv_sec) + (double)(t2.tv_nsec - t1.tv_nsec) / BILLION;
    printf("processed block %d in %.1f sec. with result %d \n", block_idx, time, found);
  }

  clock_gettime(CLOCK_REALTIME, &t_search_end);
  search_time = (double)(t_search_end.tv_sec - t_search_start.tv_sec) + (double)(t_search_end.tv_nsec - t_search_start.tv_nsec) / BILLION;
  ...
  total_time = (double)(t_end.tv_sec - t_start.tv_sec) + (double)(t_end.tv_nsec - t_start.tv_nsec) / BILLION;

  printf("\n# TIMING: setup+init = %.3f sec, search = %.3f sec, total = %.3f sec (rest = %.3f sec)\n",
         setup_time, search_time, total_time, total_time - search_time);

  printf("CSV,%lu,%lu,%.3f,%.3f,%.3f\n",
         block_addr_bit, block_addr_size, setup_time, search_time, total_time);
...
void options(int argc, char *argv[])
{
  ...
  while ((i = getopt(argc, argv, "a:m:t:g:h")) != -1)
  {
    ...
    case 'm':
      ...
      break;
    case 'g':
      generate_modulus(strtol(optarg, NULL, 10));
      exit(1);
    case 'h':
      usage(argv);
      exit(1);
